...
{
    //Enums to define what the player can do when hitting bounds
    ...
    public void Update()
    {
        ...
        if (Below != BoundsBehavior.Nothing &&  transform.position.y - colliderSize.y < Bounds.bounds.min.y)
            ApplyBoundsBehavior(Below, new Vector2(transform.position.x, Bounds.bounds.min.x + colliderSize.y));
    ...
    private void ApplyBoundsBehavior(BoundsBehavior behavior, Vector2 constraintPosition)
    {
